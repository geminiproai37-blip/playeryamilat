...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Automatically go fullscreen on load only if it's the first visit
  if (!hasVisitedBefore) {
    toggleFullscreen(document.documentElement); // Request fullscreen on the entire document
  ...
  function loadSource(server) {
    ...
    if (server.mp4) {
      ...
      dom.video.play(); // Automatically play the video
      ...
      console.log("MP4 video source set and playing.");
  ...
  dom.video.addEventListener("canplay", () => {
    ...
    // Only show poster if video is paused at the beginning
    if (dom.video.paused && dom.video.currentTime === 0) {
      dom.poster.classList.remove("hidden");
    } else {
      dom.poster.classList.add("hidden"); // Hide poster if video is not at 0:00 or is playing
    }
    showControls();
  });
  ...
  dom.playPauseBtn.addEventListener("click", () => {
    if (dom.video.paused) {
      dom.video.play();
      // Request fullscreen when playing the video, if not already in fullscreen AND it's the first visit
      const hasVisitedBefore = localStorage.getItem("hasVisitedBefore");
      if (!document.fullscreenElement && !hasVisitedBefore) {
  ...
  dom.backBtn.addEventListener("click", () => {
    ...
    if (goBackId) {
      ...
      // Fallback to a simulated home action if no history
